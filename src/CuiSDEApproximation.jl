...
# CTMC Approximation for Stochastic Differential Equations
    using Random
    using Distributions
    using LinearAlgebra
    using Plots
...
function initialize_plot()
    default(titlefont=font(12, "Arial"), guidefont=font(10, "Arial"), legendfont=font(8, "Arial"), tickfont=font(8, "Arial"))
    plot(xlabel="Strike Price", ylabel="Option Price", legend=:topright, title="European Call Option Prices vs Strike")
end
function plot_call_price_vs_strike(S0, V0, params::Dict, T, M, N, mapping_function_S, mapping_function_V, strike_range::AbstractVector{Float64}; risk_free_rate=0.0, use_krylov=true)
    ...
    if use_krylov
        plot!(strike_range, prices, label="Call Price with Krylov", xlabel="Strike", ylabel="Option Price", title="European Call Price vs Strike", legend=:topright)
    else
        plot!(strike_range, prices, label="Call Price with Matrix Exponentiation", xlabel="Strike", ylabel="Option Price", title="European Call Price vs Strike", legend=:topright)
    end
...
strike_range = range(4.0, 15.0, length=51)
initialize_plot()
T, M, N = 1.0, 50, 50
plot_call_price_vs_strike(S0, V0, params, T, M, N, mapping_function_S, mapping_function_V, strike_range; risk_free_rate=0.0)
plot_call_price_vs_strike(S0, V0, params, T, M, N, mapping_function_S, mapping_function_V, strike_range; risk_free_rate=0.0, use_krylov=false)
