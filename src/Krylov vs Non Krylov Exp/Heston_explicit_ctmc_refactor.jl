...
#   dV_t = kappa V_t*(theta - V_t) dt + xi * V_t * sqrt(V_t) dW_t^{(2)}
#   corr(dW^{(1)}, dW^{(2)}) = rho
#
...
@with_kw mutable struct ThreeTwoParams
    ...
    kappa::Float64 = 22.84 # mean reversion speed for variance
    theta::Float64 = 0.48 # long‑run mean variance
    xi::Float64    = 8.56 # vol‑of‑vol (ξ)
    rho::Float64   = -0.99 # correlation
    v0::Float64    = 0.06 # initial variance
end

# Helpers for CIR drift/diffusion
threetwo_drift(v, p::ThreeTwoParams) = p.kappa * v *(p.theta - v)
threetwo_sigma(v, p::ThreeTwoParams) = p.xi* v * sqrt(max(v, 0.0))
...
function construct_generator_threetwo(v::Vector{Float64}, p::ThreeTwoParams;
    ...
    μ⁺(x) = max(threetwo_drift(x, p), 0.0)
    μ⁻(x) = max(-threetwo_drift(x, p), 0.0)
    σ²(x) = threetwo_sigma(x, p)^2
...
"""
function gp_params(J::Real, ΔV::Real, T::Real, p::ThreeTwoParams; use_user_notation::Bool=false)
    if use_user_notation
        # User manuscript form (as given in earlier notes):
        μ = (p.r - p.rho * p.theta / p.kappa) * T + (p.rho * p.xi / p.kappa - 0.5) * J + (p.rho / p.kappa) * ΔV
        # NB: this line interprets manuscript symbols heuristically; prefer canonical form below.
    else
        # Canonical Heston GP moments
        μ = (p.r - (p.rho * p.kappa * p.theta) / p.xi) * T + ((p.rho * p.kappa)/p.xi - 0.5) * J + (p.rho / p.xi) * ΔV
    end
    σ2 = (1 - p.rho^2) * J
    return μ, σ2
end
...
"""
function bs_call_kernel(S0::Real, K::Real, r::Real, μ::Real, σ2::Real, T::Real)
    σ = sqrt(max(σ2, 0.0))
    @assert σ > 0 "Zero conditional variance: handle separately"
    d1 = (log(S0/K) + r*T + 0.5*σ2) / σ
    d2 = d1 - σ
    Φ = cdf.(Normal(), [d1, d2])
    return S0*Φ[1] - K*exp(-r*T)*Φ[2]
end

function bs_put_kernel(S0::Real, K::Real, r::Real, μ::Real, σ2::Real, T::Real)
    σ = sqrt(max(σ2, 0.0))
    @assert σ > 0 "Zero conditional variance: handle separately"
    d1 = (log(S0/K) + r*T + 0.5*σ2) / σ
    d2 = d1 - σ
...
function price_threetwo_ctmc_mc(p::ThreeTwoParams, vgrid::Vector{Float64}, Q::Matrix{Float64},
    ...
    @inbounds for _ in 1:npaths
        ...
        μ, σ2 = gp_params(J, ΔV, T, p; use_user_notation)
        acc += bs_call_kernel(S0, K, r, μ, σ2, T)
    end
    return acc / npaths
end
...
function simulate_threetwo_QE(p::ThreeTwoParams, T::Float64; nsteps::Int=252, npaths::Int=10000, path::Bool=false)
    Δt = T/nsteps
    S = fill(p.S0, npaths)
    V = fill(p.v0, npaths)
...
# PS_1 PS1 1 1 2 1.5 0.2 −0.5 0.05
p = ThreeTwoParams(S0=1.0, r=0.05, kappa=2.0, theta=1.5, xi=0.2, rho=-0.5, v0=1.0)
...
T, K = 1.0, 1.0
price_mc = price_threetwo_ctmc_mc(p, vgrid, Q, T, K; npaths=10000)
...
PS_Cui = HestonParams(S0=10.0, r=0.0, kappa=4.0, theta=0.035, xi=0.15, rho=-0.75, v0=0.04)
T = 1.0
K = 4.0

v_min, v_max, m = 1e-2 * PS_Cui.v0, 8.0 * PS_Cui.v0, 5000
vgrid = collect(range(v_min, v_max, length=m))
Q = construct_generator_CIR(vgrid, PS_Cui, tolerance = 1e-5)
@info "Q shape" size(Q)
@info "Row sums (∞‑norm) ~ 0" maximum(abs.(sum(Q, dims=2)))

price_mc_cui = price_heston_ctmc_mc(PS_Cui, vgrid, Q, T, K, npaths=1000)
...
println(errors)
plot(grid_sizes, , xlabel="N", ylabel="Absolute Error", title="Convergence of European Option Price", legend=false)


### 4/2 params 

PS_Cui_2 = HestonParams(S0=100.0, r=0.05, kappa=1.8, theta=0.04, xi=0.1, rho=-0.7, v0=0.04)
K = 100.0
T = 1.0
v_min, v_max, m = 1e-3 * PS_Cui_2.v0, 40.0 * PS_Cui_2.v0, 1000
vgrid = collect(range(v_min, v_max, length=m))
Q = construct_generator_CIR(vgrid, PS_Cui_2; tolerance=1e-5)
@info "Q shape" size(Q)
@info "Row sums (∞‑norm) ~ 0" maximum(abs.(sum(Q, dims=2)))

price_mc_cui_2 = price_heston_ctmc_mc(PS_Cui_2, vgrid, Q, T, K, npaths=100)


### 2nd 4/2 params η= 1, θ= 0.025, v0 = 0.025, ρ=−0.7, σv = 0.18.

PS_Cui_2b = HestonParams(S0=100.0, r=0.05, kappa=1.8, theta=0.025, xi=0.18, rho=-0.7, v0=0.025)
K = 100.0
T = 0.5
v_min, v_max, m = 1e-3 * PS_Cui_2b.v0, 40.0 * PS_Cui_2b.v0, 1000
vgrid = collect(range(v_min, v_max, length=m))
Q = construct_generator_CIR(vgrid, PS_Cui_2b; tolerance=1e-5)
@info "Q shape" size(Q)
@info "Row sums (∞‑norm) ~ 0" maximum(abs.(sum(Q, dims=2)))

price_mc_cui_2b = price_heston_ctmc_mc(PS_Cui_2b, vgrid, Q, T, K, npaths=100)

## American option pricing baselines for the heston models
